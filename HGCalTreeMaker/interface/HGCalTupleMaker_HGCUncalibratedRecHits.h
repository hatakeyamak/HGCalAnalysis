#ifndef HGCalTupleMaker_HGCUncalibratedRecHits_h
#define HGCalTupleMaker_HGCUncalibratedRecHits_h
...
class HGCalTupleMaker_HGCUncalibratedRecHits : public edm::EDProducer {
 protected:

  std::vector<edm::InputTag> m_HGCRecHitsTags;
  std::vector<edm::EDGetTokenT<HGCUncalibratedRecHitCollection>> m_HGCRecHitsTokens;
  ...
  void produce( edm::Event & iEvent, const edm::EventSetup & iSetup) { 
    ...
    edm::Handle<HGCUncalibratedRecHitCollection> HGCRecHits;

    //
    // Loop over HGCUncalibratedRecHitCollections
    //
    for( typename std::vector<edm::EDGetTokenT<HGCUncalibratedRecHitCollection> >::const_iterator
	   token = m_HGCRecHitsTokens.begin(); token != m_HGCRecHitsTokens.end(); ++token ) {
      unsigned index(token - m_HGCRecHitsTokens.begin());
      HGCRecHits.clear();

      if (debug) std::cout << "HGCalTupleMaker_HGCUncalibratedRecHits: " << index << std::endl;
 ...
 HGCalTupleMaker_HGCUncalibratedRecHits(const edm::ParameterSet& iConfig) :
  m_HGCRecHitsTags (iConfig.getUntrackedParameter<std::vector<edm::InputTag> >("source")),
    m_HGCRecHitsTokens(edm::vector_transform(m_HGCRecHitsTags, [this](edm::InputTag const & tag){
	  return consumes<HGCUncalibratedRecHitCollection>(tag);})),
    ...
    produces<std::vector<float> > ( m_prefix + "Amplitude" + m_suffix );
  ...
  std::unique_ptr<std::vector<float> > amplitude;
 ...
 protected:
  ...
  void loadAlgo(){
    ...
    amplitude = std::unique_ptr<std::vector<float> > ( new std::vector<float> ());
  ...
  void dumpAlgo( edm::Event & iEvent ){
    ...
    iEvent.put( move(amplitude ), m_prefix + "Amplitude" + m_suffix );
